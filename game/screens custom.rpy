    ...
    if BM.weaponhover != None and BM.show_tooltips and BM.active_weapon == None:
        ...
        if weapon.tooltip != None:
            frame:
                ...
                text unicode(str(weapon.tooltip),'utf8'):
...
screen victory2:
    ...
    $wait += 0.5
    text 'Nagroda za pokonane jednostki wroga: {}$'.format(int(store.total_money)):
        xpos 0.1
        ypos 0.6
        size 40
        outlines [(2,'000',0,0)]
    ...
    if store.surrender_bonus > 0:
        $wait += 0.1
        text 'Bonus za poddane jednostki wroga: {}$'.format(int(store.surrender_bonus)):
            xpos 0.1
            ypos yposition
            size 40
            outlines [(2,'000',0,0)]
    ...
    $wait += 0.1
    text 'Koszty napraw: {}$'.format(int(store.repair_cost)):
        xpos 0.1
        ypos yposition
        size 40
        outlines [(2,'000',0,0)]
    ...
    $wait += 0.1
    text 'Przychód po odliczeniu kosztów: {}$'.format(int(store.net_gain)):
        xpos 0.1
        ypos yposition
        size 40
        outlines [(2,'000',0,0)]
    ...
    $wait += 0.1
    text 'Ilość tur: {}'.format(BM.turn_count):
        xanchor 1.0
        xpos 0.85
        ypos 0.65
        size 40
    ...
    $ diff_text = "Obecna trudność: {}".format( DIFFICULTY_NAMES[store.Difficulty] )
    $ low_diff_text = "Najniższa trudność: {}".format( DIFFICULTY_NAMES[BM.lowest_difficulty] )
